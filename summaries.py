...
#  editing.checker() 
  ...
  finished = False
  while not finished:
    pat_p = re.search(r'\[\[(.+?)\|', search_string[m_pt:]) # remove [[ --- ]] links, but preserve the link text
    if pat_p != None:
      en_pt = pat_p.end() + m_pt
      st_pt = pat_p.start() + m_pt
#      print("snipping [[ ... |",str(s_pt),str(m_pt))  
      search_string = editing.snip(st_pt,en_pt)
      m_pt = st_pt
      pat_p1 = re.search(r'(.+?)\]\]', search_string[m_pt:]) # remove  --- ]] part of link, but preserve the link text
      if pat_p1 != None:
        en_pt = pat_p1.end() + m_pt
        st_pt = pat_p1.start() + m_pt
        search_string = editing.snip(en_pt-2,en_pt)
        m_pt = en_pt - 2
      else:
        print("Could not find ]] at end of link!")
    else:
      finished = True
...
for page_entry in page_list:
  ...
  n_pages += 1
  if len(items) < 6:
    print("Pages file entry has less than 6 items:\n  ",str(items))
  elif items[5] == "":  
    print("Summarising ",items[1])
    success = True
    try:  
      page_name = items[1]  
